...
namespace Angaraka::Core {
    ...
    struct AssetDefinition {
        ...
        std::string id;           // Unique identifier (e.g., "ui/button_normal")
        std::string path;         // File path relative to assets folder
    ...
    struct AssetBundleConfig {
        ...
        // Helper methods
        bool HasDependency(const std::string& bundleName) const;
        std::vector<AssetDefinition> GetAssetsByPriority() const;  // Returns assets sorted by priority
        float GetLoadProgress() const;                             // Returns 0.0-1.0 load progress
    };
